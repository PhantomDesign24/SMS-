...
function send_sms($phone, $message, $type = 'etc') {
    ...
    if($g5_sms_config['cf_service'] == 'icode') {
        ...
        // 1. SMS5 플러그인 확인 (최신)
        if(defined('G5_SMS5_USE') && G5_SMS5_USE && file_exists(G5_PLUGIN_PATH.'/sms5/sms5lib.lib.php')) {
            include_once(G5_PLUGIN_PATH.'/sms5/sms5lib.lib.php');
        ...
        // 2. 구버전 아이코드 라이브러리 확인
            ...
            // 발송
            $recv_list = $phone;
            $send_list = array($g5_sms_config['cf_phone']);
